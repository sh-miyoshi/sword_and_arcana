...
world.afterEvents.itemStopUse.subscribe(event => {
  ...
  system.run(() => {
    if (charge && player.isValid && charges.get(player.id) === charge)
      cancelCharge(player)
  })
})
...
world.afterEvents.playerDimensionChange.subscribe(event =>
  cancelCharge(event.player)
)
world.afterEvents.entityDie.subscribe(event => {
  if (event.deadEntity.typeId === 'minecraft:player')
    cancelCharge(event.deadEntity)
})
...
function finishCharge (player) {
  ...
  try {
    dimension.playSound('ambient.weather.thunder', origin, {
      volume: 0.7,
      pitch: 1.1
    })
  } catch (error) {
...
function damageLine (player, dimension, origin, forward, right) {
  const targets = dimension.getEntities({
    ...
    maxDistance: Math.hypot(
      HIT_RANGE,
      HIT_HALF_WIDTH,
      Math.max(-HIT_MIN_HEIGHT, HIT_MAX_HEIGHT)
    )
  })
  for (const target of targets) {
    try {
      if (
        ...
        target.typeId === 'minecraft:armor_stand' ||
      ...
      if (
        distanceForward <= 0 ||
        distanceForward > HIT_RANGE ||
        Math.abs(distanceSide) > HIT_HALF_WIDTH ||
        dy < HIT_MIN_HEIGHT ||
        dy > HIT_MAX_HEIGHT
      )
        continue
  ...
  try {
    ...
    for (const height of [0.3, 1.0, 1.7]) {
      ...
      for (let segment = 1; segment <= BOLT_SEGMENTS; segment++) {
        ...
        const side =
          segment === BOLT_SEGMENTS
            ? 0
            : (Math.random() - 0.5) * HIT_HALF_WIDTH * 1.4
        const next = {
          ...
          for (const sign of [-1, 1]) {
            const branchEnd = {
              x:
                origin.x +
                forward.x * distance +
                right.x * HIT_HALF_WIDTH * 0.85 * sign,
              y: origin.y + 1 + sign * 0.7,
              z:
                origin.z +
                forward.z * distance +
                right.z * HIT_HALF_WIDTH * 0.85 * sign
            }
