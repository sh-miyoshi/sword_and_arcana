...
system.beforeEvents.startup.subscribe(event => {
  event.itemComponentRegistry.registerCustomComponent('my:ice_sword_charge', {
    onCompleteUse (event) {
...
world.afterEvents.itemStartUse.subscribe(event => {
  ...
  if (!item || item.typeId !== ICE_SWORD_ID) {
    return
  }
...
system.runInterval(() => {
  for (const player of world.getAllPlayers()) {
    ...
    if (!heldItem || heldItem.typeId !== ICE_SWORD_ID) {
      chargeStartTicks.delete(player.id)
      setActionChargeCount(0, player)
      continue
    }
...
function iceSwordChargeAttack (player) {
...
function spawnIceSlash (player, forward) {
  ...
  for (let angle = -70; angle <= 70; angle += 7) {
    ...
    dimension.spawnParticle('my:ice_slash', particlePos)
  }
  ...
  for (const target of targets) {
    ...
    if (target.isValid) {
      freezeEntity(target, FREEZE_DURATION_TICKS)
    }
  ...
  showIceSlashHitbox(player, forward)
  if (DEBUG_HITBOX) {
    player.sendMessage(`氷斬撃の命中数: ${targets.length}`)
  }
}

function getIceSlashTargets (player, forward) {
  const origin = player.location
...
function showIceSlashHitbox (player, forward) {
  const dimension = player.dimension
  const origin = player.location
  const right = {
    x: -forward.z,
  ...
  for (let distance = 1; distance <= HIT_RANGE; distance += 1) {
    for (let angle = -HIT_HALF_ANGLE; angle <= HIT_HALF_ANGLE; angle += 14) {
      ...
      dimension.spawnParticle('my:ice_slash', hitPosition)
    }
  }
}
